...
Run `$yeet` and `$yoink` as one uninterrupted delivery step. Treat invocation of `$sendit` as authorization to open the PR and continue through actual merge completion once the merge gates are satisfied.
